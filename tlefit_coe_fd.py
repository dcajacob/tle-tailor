...
from common_coe_fd import *

import coarse_fit
from coarse_fit import coarse_fit
...
def test_tle_fit(
    ...
    """Use an existing TLE to fit a matching TLE. This is mostly to demonstrate the algorithm works, since we have a known good solution.
    ...
    for x in range(max_iter):
        ...
        while True:  # Try adjusting lamda until we converge
            ...
            for offset in offsets:
                ...
                for idx, element in enumerate(elements):
                    ...
                    if idx == 6:  # B* is a snowflake
                        ...
                        delta_amt, _ = finite_diff(
                            element,
                            percent_chg=percent_chg,
                            delta_amt_chg=delta_amt_chg,
                        )
            ...
            if debug:
                ...
                if lamda:
                    print(
                        f"Condition number (ATWA_acc): {np.linalg.cond(ATWA_acc + lamda * np.eye(7))}"
                    )
                else:
        ...
        if debug and lamda:
            print("Lambda: ", lamda)
            print(
                f"Residuals after/before {res_new:0.3g} {'<' if res_new < res_old else '>'} {res_old:0.3g}"
            )

        if debug:
            print(f"Covariance a: {np.sqrt(np.diag(P)[0]) * 1000:0.3f} m")
        ...
        if coe_limit:
            ...
            # Limit b*
            x_new[6] = np.clip(x_new[6], -1, 1)

        # # Also does the x_new = elements + dx
        # x_new, dx = limit_elements(elements, dx)
        ...
        if debug:
            print(f"COE elements = {x_new}")

            print(
                f"\nCurrent Solution {elements[0]} {elements[1]:0.7f} {np.degrees(elements[2]):3.4f} {np.degrees(elements[3]):3.4f} {np.degrees(elements[4]):3.4f} {np.degrees(elements[5]):3.4f} {elements[6]:+1.4e}"
            )
            print(
        ...
        dx_test = (
            np.max(np.abs(dx / elements)) < rms_epsilon and sigma_new < rms_epsilon
        )
        ...
        if debug:
            ...
            print(
                f"Convergence test: {convergence_test:0.6g}, sigma_new({sigma_new:0.3g}) < sigma_old({sigma_old:0.3g})"
            )

        # if (convergence_test <= rms_epsilon or sigma_new == sigma_old) and sigma_new <= sigma_old and sigma_new < rms_epsilon:# or x > 0 and sigma_new > sigma_old:
        ...
        if dx_test:
            ...
            if debug:
                ...
                if (
                    residual_test < rms_epsilon
                ):  # np.abs(res_new - res_old) < rms_epsilon:
            ...
            if debug:
                ...
                print(
                    f"Solution {elements[0]} {elements[1]:0.7f} {np.degrees(elements[2]):3.4f} {np.degrees(elements[3]):3.4f} {np.degrees(elements[4]):3.4f} {np.degrees(elements[5]):3.4f} {elements[6]:+1.4e}"
                )
                print(
                    f"Original {orig_elements[0]} {orig_elements[1]:0.7f} {np.degrees(orig_elements[2]):3.4f} {np.degrees(orig_elements[3]):3.4f} {np.degrees(orig_elements[4]):3.4f} {np.degrees(orig_elements[5]):3.4f} {orig_elements[6]:+1.4e}"
                )
...
def test_tle_fit_normalized(
    ...
    for x in range(max_iter):
        ...
        if debug:
            print("Covariance a: %0.3f m" % (np.sqrt(np.diag(P)[0]) * 1000))
        ...
        if debug:
            ...
            print(
                f"Convergence test: {convergence_test:0.6g}, sigma_new({sigma_new:0.3g}) {'<' if sigma_new < sigma_old else '>'} sigma_old({sigma_old:0.3g})"
            )
